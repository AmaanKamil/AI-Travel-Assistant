    ...
    return new Promise((resolve, reject) => {
        ...
        try {
            ...
            const doc = new PDFDocument({
                margin: 50,
                size: 'A4',
                bufferPages: true // Keep for safe range checks if needed, though we won't use switchToPage
            });
            ...
            // Helper for Footer (Sequential only)
            const drawFooter = () => {
            ...
            itinerary.days.forEach((day, index) => {
                ...
                day.blocks.forEach((block: any, blockIdx: number) => {
                    ...
                    if (cardY + cardHeight > 750) {
                        console.log(`[PDF] Adding new page. Current page count: ${doc.bufferedPageRange().count}`);
                        drawFooter(); // Draw on current page before adding new one
                        doc.addPage();
                    }
                    ...
                    doc.fontSize(9)
                        .font('Helvetica-Bold')
                        .fillColor(isMeal ? '#059669' : colors.primary)
            ...
            stream.on('finish', () => {
                console.log(`[PDF] PDF successfully generated at: ${filePath}`);
                resolve(filePath);
            });
            stream.on('error', err => {
            ...
            console.error('[PDF] Unexpected crash during generation:', err.message);
            reject(err);
        }
    });
}
