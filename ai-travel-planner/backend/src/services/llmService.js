...
function extractIntent(text, currentState) {
    return __awaiter(this, void 0, void 0, function () {
        var start, normalized, extractDaysRegex, performFallback, systemPrompt, completion, content, parsed, regexDays, dayMatch, error_1;
        var _a, _b;
        return __generator(this, function (_c) {
            switch (_c.label) {
                case 0:
                    start = Date.now();
                    console.log("[LLM Service] Parsing: \"".concat(text, "\" | State: ").concat(currentState || 'N/A'));
                    if (!text || text.trim().length === 0)
                        return [2 /*return*/, { type: 'plan_trip', entities: {} }];
                    ...
                    if (currentState === 'CONFIRMING') {
                        if (normalized.includes('yes') ||
                            ...
                            return [2 /*return*/, { type: 'CONFIRM_GENERATE' }];
                        }
                    }
                    if (normalized === 'yes' ||
                        normalized === 'yeah' ||
                    ...
                    if (/^(yes|yeah|yep|sure|go ahead|generate|okay|ok)$/i.test(text.trim())) {
                        return [2 /*return*/, { type: 'CONFIRM_GENERATE', entities: {} }];
                    }
                    extractDaysRegex = function (input) {
                        var match = input.match(/(\d+)\s*days?/i) || input.match(/^(\d+)$/);
                        return match ? parseInt(match[1]) : null;
                    };
                    performFallback = function (input) {
                        console.log("[LLM Service] Using Fallback Logic for: \"".concat(input, "\""));
                        var lower = input.toLowerCase();
                        // Check days explicitly
                        var days = extractDaysRegex(input);
                        if (days)
                            return { type: 'plan_trip', entities: { days: days } };
                        ...
                        if (lower.includes("why"))
                            return { type: 'ask_question', entities: {} };
                        if (lower.includes("email") || lower.includes("send"))
                            return { type: 'export', entities: {} };
                        return { type: 'plan_trip', entities: {} };
                    };
                    if (!openai) return [3 /*break*/, 5];
                    _c.label = 1;
                case 1:
                    _c.trys.push([1, 3, , 4]);
                ...
                case 2:
                    ...
                    console.log("[LLM Service] Success (".concat(Date.now() - start, "ms)"));
                    regexDays = extractDaysRegex(text);
                    if (regexDays && (!parsed.entities || !parsed.entities.days)) {
                        parsed.entities = __assign(__assign({}, (parsed.entities || {})), { days: regexDays });
                        // Also force type if ambiguously parsed
                        if (!parsed.type)
                            parsed.type = 'plan_trip';
                    }
                ...
                case 3:
                    error_1 = _c.sent();
                    console.warn("[LLM Service] OpenAI Failed: ".concat(error_1.message, ". Switching to Fallback."));
                    return [2 /*return*/, performFallback(text)];
